 ...
 * it. **The `accounts` calls stay named**, and that is not an oversight: `mica_accounts`
 ...
 *
 * A convar (`mica_max_accounts_per_app`) the phone cannot read, so it arrives with the account
 ...
 * `mica_notifications` row for the same event, the launcher badge summed both and one mention
 ...
 *
 * `mica_account_reactions` lives on the shared `accounts` service (§10's "the accounts
 ...
 */
const DM_TARGET_TABLE = 'mica_blabber_dms';
