 ...
 * count — and the server persists a `mica_notifications` row for the very mentions and DMs the
 ...
 *
 * It emits `0` synchronously and swaps in the real store only once `import('@mica/sdk')` and
...
beforeAll(async () => {
  ...
  // `import('@mica/sdk')` is much slower than the composition itself, and warming it here is
  // what keeps these tests measuring the badge rather than the module loader.
  await import('@mica/sdk');
  await import('./store');
  await readBadge();
});
