...
/**
 * MICA-172 moved this file out of `sdk/` and into `web/src/`, and `@mica/sdk/testing` is
 ...
 * boundary. Its own docblock below already said it is not exported from `@mica/sdk`; the
 * specifier is unchanged and all four consumers are `web/` app tests.
 *
 * MICA-176. `@mica/sdk/testing` is an entry point like `src/main.ts` and `bootAddOn`,
...
/**
 * Test-only SDK surface. **Not exported from `@mica/sdk`** — importing this pulls in
 * `@testing-library/svelte` and `vitest`, neither of which belongs in a phone that
 * ships. Import it as `@mica/sdk/testing`, which is aliased for exactly this.
 *
