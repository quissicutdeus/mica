 ...
 *   `mica://messages?threadId=1` what the browser mocks wrote
 ...
 * ## The format
 *
 * `app?key=value&key=value`, with an optional `mica://` prefix that is stripped.
 *
...
export function parseDeepLink(link: string): DeepLink | null {
  ...
  const withoutScheme = link.trim().replace(/^mica:\/\//i, '');
