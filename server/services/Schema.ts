...
/**
 * `micaschema apply` — actually change the database.
 *
...
export const runApply = async (source: number): Promise<void> => {
  if (source !== 0) {
    console.log('[micaschema] apply only runs from the server console.');
    return;
  }
  ...
  if (result.failed) {
    console.error(`[mica] migration ${result.failed.id} failed: ${result.failed.error}`);
    if (result.remaining.length > 0) {
      console.error(`[mica] not attempted: ${result.remaining.join(', ')}`);
    }
    console.error('[mica] additive changes were not applied — fix the migration first.');
    return;
  }

  const additive = await SchemaMigrator.apply();
  for (const line of additive.applied) console.log(`[mica] ${line}`);
  if (additive.failed) {
    console.error(`[mica] ${additive.failed.description} failed: ${additive.failed.error}`);
    if (additive.remaining.length > 0) {
      console.error(`[mica] not attempted: ${additive.remaining.join(', ')}`);
    }
    return;
  }
...
/**
 * `micaschema` — print what would change, without changing it.
 *
  ...
  (source: number, args: string[]) => {
    ...
    if ((args?.[0] ?? '').toLowerCase() === 'apply') {
      void runApply(source).catch((error) => {
        console.error('[micaschema] apply failed:', error);
      });
      return;
    }
