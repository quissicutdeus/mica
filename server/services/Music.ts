 ...
 *
 * **So the answer to "does this survive `ensure gphone`" is no, deliberately.** A restart
 ...
 * `ensure gphone`. One short string per character who has ever pressed play.
 */
...
/** Read-only view of the live state, for tests and for `gphonemusic`-style diagnostics. */
...
const rangeMeters = (): number =>
  typeof GetConvarInt === 'function'
    ? GetConvarInt('gphone_music_range', DEFAULT_MUSIC_RANGE)
    : DEFAULT_MUSIC_RANGE;
...
const maxNearby = (): number => {
  const raw =
    typeof GetConvarInt === 'function'
      ? GetConvarInt('gphone_music_max_nearby', DEFAULT_MAX_NEARBY)
      : DEFAULT_MAX_NEARBY;
...
const send = (src: number, envelope: NearbyMusicEnvelope): boolean => {
  ...
  try {
    ...
    console.error(`[gphone] music push to ${src} failed:`, error);
    return false;
  }
};
