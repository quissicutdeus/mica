...
/**
 * `micaseed` — populate a dev server with someone to talk to.
 *
...
/** `micaseed text <firstname> <message…>` — have a seeded character text you. */
const runText = async (source: number, owner: string, args: string[]): Promise<void> => {
  ...
  if (!body) {
    respond(source, 'Usage: micaseed text <firstname> <message>', 'error');
    return;
  }
  ...
  if (!conversationId) {
    respond(source, 'No thread with them yet — run micaseed first.', 'error');
    return;
  }
...
const runSeedCommand = async (source: number, args: string[]): Promise<void> => {
  ...
  if (sub && sub !== 'add') {
    respond(source, 'Usage: micaseed [add | text <firstname> <message> | clear]', 'error');
    return;
  }
...
RegisterCommand(
  ...
  (source: number, args: string[]) => {
    void runSeedCommand(source, args ?? []).catch((error) => {
      console.error('[micaseed] failed:', error);
