...
/**
 * MICA-151: the numbers gPhone issues when nothing else will.
 *
 ...
 * the rest of the repo already uses, and clear of the block `gphoneseed` owns.
 *
...
describe('the generated number', () => {
  ...
  it('never lands in the exchange `gphoneseed` owns', () => {
  ...
  it('is not derived from the citizenid', () => {
    ...
    // shown, and hands the same player the same number on every server running gPhone.
...
describe('telling a duplicate key from a real failure', () => {
  ...
  it.each([
    ['a table that was never imported', new Error("Table 'gphone_phone_numbers' doesn't exist")],
...
describe('assigning a number', () => {
  ...
  it('stops at once on a failure that is not a duplicate, and names the table', async () => {
    dbMock.insert.mockRejectedValue(new Error("Table 'gphone_phone_numbers' doesn't exist"));
...
describe('the declaration', () => {
  it('registers no net event, so a payload can never reach this table', () => {
    ...
    const mine = registered.filter((call: unknown[]) =>
      String(call[0]).startsWith('gphone:server:phonenumbers:')
    );
...
describe('when a number is assigned', () => {
  ...
  it('assigns nothing on a framework server, whose numbers gPhone does not own', async () => {
    ...
    // reads, and a gPhone number beside it would be one only the phone believed.
