...
class NoUpdatedAtRepo extends Repository<{ id: number; citizenid: string; status: string }> {
  protected tableName = 'mica_no_updated_at';
...
class NoOwnerRepo extends Repository<{ id: number; label: string; status: string }> {
  protected tableName = 'mica_global';
...
describe('Repository — SQL identifier allowlist', () => {
  ...
  it('quotes identifiers and keeps every value parameterized, in key order', async () => {
    ...
    expect(sqlOf(dbMock.insert.mock.calls[0])).toBe(
      'INSERT INTO `mica_test` (`title`, `content`) VALUES (?, ?)'
    );
...
describe('Repository — ownership scoping', () => {
  ...
  it('omits the ownership predicate only through the protected escape hatch', async () => {
    ...
    expect(sqlOf(dbMock.update.mock.calls[0])).toBe(
      'UPDATE `mica_test` SET `status` = ? WHERE `id` = ?'
    );
...
describe('Repository — soft delete', () => {
  ...
  it('deletes by moving status, scoped to the owner', async () => {
    ...
    expect(sqlOf(dbMock.update.mock.calls[0])).toBe(
      "UPDATE `mica_test` SET `status` = ? WHERE `id` = ? AND `citizenid` = ? AND `status` != 'moderated'"
    );
...
describe('Repository — restore (MICA-75)', () => {
  ...
  it('undoes a delete, scoped to the owner and bounded to the window', async () => {
    ...
    expect(success).toBe(true);
    expect(sqlOf(dbMock.update.mock.calls[0])).toBe(
      "UPDATE `mica_test` SET `status` = 'active' " +
...
describe('Repository — findDeleted (MICA-75-wiring)', () => {
  ...
  it('projects down to given columns, the same way findAll does, so a heavy column is opt-in', async () => {
    ...
    expect(sqlOf(dbMock.query.mock.calls[0])).toBe(
      'SELECT `id`, `title`, `updated_at` FROM `mica_test` ' +
        "WHERE `citizenid` = ? AND `status` = 'deleted' " +
        'AND `updated_at` >= NOW() - INTERVAL ? DAY ' +
        'ORDER BY `updated_at` DESC'
    );
...
describe('Repository — reads', () => {
  ...
  it('defaults findAll to active rows', async () => {
    ...
    expect(sqlOf(dbMock.query.mock.calls[0])).toBe(
      'SELECT * FROM `mica_test` WHERE `citizenid` = ? AND `status` = ?'
    );
  ...
  it('scopes findById by owner when a citizenid is supplied', async () => {
    ...
    expect(sqlOf(dbMock.single.mock.calls[0])).toBe(
      'SELECT * FROM `mica_test` WHERE `id` = ? AND `citizenid` = ?'
    );
  ...
  it('leaves findById unscoped for server-internal reads', async () => {
    ...
    expect(sqlOf(dbMock.single.mock.calls[0])).toBe('SELECT * FROM `mica_test` WHERE `id` = ?');
...
describe('Repository: keyset paging', () => {
  ...
  it('orders by the primary key descending, and limits, when paged', async () => {
    ...
    expect(sql).toBe('SELECT * FROM `mica_test` WHERE `status` = ? ORDER BY `id` DESC LIMIT ?');
