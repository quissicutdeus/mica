...
const call = async (action: string, data: unknown) => {
  const handler = handlers.get(`gphone:server:notes:${action}`);
...
describe('notes:restore (MICA-75)', () => {
  it('restores a deleted note within the window, scoped to the caller citizenid', async () => {
    ...
    expect(String(sql)).toContain('UPDATE `gphone_notes`');
  ...
  it('leaves the generic CRUD actions registered alongside it', () => {
    expect(handlers.has('gphone:server:notes:restore')).toBe(true);
    expect(handlers.has('gphone:server:notes:get')).toBe(true);
    expect(handlers.has('gphone:server:notes:create')).toBe(true);
    expect(handlers.has('gphone:server:notes:update')).toBe(true);
    expect(handlers.has('gphone:server:notes:delete')).toBe(true);
...
describe('notes:getDeleted (MICA-75-wiring)', () => {
  it('reads only the caller’s own deleted notes, bounded to the restore window', async () => {
    ...
    expect(String(sql)).toContain('FROM `gphone_notes`');
  ...
  it('is registered alongside restore', () => {
    expect(handlers.has('gphone:server:notes:getDeleted')).toBe(true);
  });
});
