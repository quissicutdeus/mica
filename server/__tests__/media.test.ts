...
const SHARE_LOCATION_EVENT = 'mica:server:media:shareLocation';
const GET_EVENT = 'mica:server:media:get';
const ITEM_EVENT = 'mica:server:media:item';
const THUMBNAIL_EVENT = 'mica:server:media:thumbnail';
const RESTORE_EVENT = 'mica:server:media:restore';
...
describe('media:drop', () => {
  ...
  it('reports only the copies the database actually wrote', async () => {
    ...
    const pushed = (globalThis.emitNet as any).mock.calls
      .filter((args: unknown[]) => args[0] === 'mica:client:shell:appEvent')
...
describe('media:get — the list read (MICA-110)', () => {
  ...
  it('refuses a cursor that is not a row id', async () => {
    const reply = await callGet({ cursor: 'id; DROP TABLE mica_media' });
 ...
 * under the default `mica_rate_limit`, into a table with no retention. `thumbnail` next
 ...
 * `mica_camera_quality`, base64 — and never from what the column tolerates.
 */
describe('media:create — the size a photo may actually be (MICA-116)', () => {
  ...
  it('still accepts one several times larger than the camera has ever produced', async () => {
    ...
    // taken on a server that set `mica_camera_quality 100`.
...
describe('media:getDeleted (MICA-75-wiring)', () => {
  const GET_DELETED_EVENT = 'mica:server:media:getDeleted';
