 ...
 *
 * `mica_hodlr.quantity` always did. The price did not, so every restart re-valued every
...
describe('HodlrMarket restore', () => {
   ...
   * exactly the constant the original exploit was built on, with `mica_hodlr.quantity`
   ...
   * `restart mica` to recover. A *rejected* read was always fine and is covered above.
   */
