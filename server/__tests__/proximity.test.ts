...
describe('findNearbyVisiblePlayers', () => {
  ...
  it('respects the mica_bluetooth_range convar', async () => {
    place(1, [0, 0, 0]);
   ...
   * caps its roster with `mica_music_max_nearby` for precisely this reason.
   */
  describe('the recipient cap (MICA-115)', () => {
    ...
    it('respects mica_bluetooth_max_nearby', async () => {
      crowd(20);
      (globalThis as any).GetConvarInt = (name: string, fallback: number) =>
        name === 'mica_bluetooth_max_nearby' ? 2 : fallback;
    ...
    it('clamps a convar raised past the ceiling', async () => {
      crowd(20);
      (globalThis as any).GetConvarInt = (name: string, fallback: number) =>
        name === 'mica_bluetooth_max_nearby' ? 999 : fallback;
    ...
    it('falls back to the default on a value that is not a positive number', async () => {
      crowd(20);
      (globalThis as any).GetConvarInt = (name: string, fallback: number) =>
        name === 'mica_bluetooth_max_nearby' ? 0 : fallback;

      // `mica_bluetooth_range` is the knob that turns proximity sharing off; a typo in
