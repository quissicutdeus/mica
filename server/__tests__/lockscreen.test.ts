...
describe('lockscreen — no generic action survives', () => {
  it('registers exactly the four named actions and nothing generic', () => {
    expect(
      [...handlers.keys()].filter((e) => e.startsWith('mica:server:lockscreen:')).toSorted()
    ).toEqual([
      'mica:server:lockscreen:check',
      'mica:server:lockscreen:clear',
      'mica:server:lockscreen:set',
      'mica:server:lockscreen:status'
    ]);
  });
});
