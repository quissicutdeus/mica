...
const { bridgeMock, convar, netHandlers, loadedSubscribers, emitNet } = vi.hoisted(() => {
  ...
  (globalThis as any).GetConvar = (name: string, fallback: string) =>
    name === 'gphone_phone_item' ? value.current : fallback;
  return {
    bridgeMock: {
      getPlayer: vi.fn(),
      countItem: vi.fn(),
...
const PUSH = 'gphone:client:shell:phoneItem';
const OPEN = 'gphone:client:shell:open';

/**
 * MICA-229: the phone opens only for a player holding the item `gphone_phone_item` names.
...
describe('the phone item gate', () => {
  ...
  it('is named by the convar the README documents', () => {
    expect(PHONE_ITEM_CONVAR).toBe('gphone_phone_item');
  ...
  describe('the "look again" event', () => {
    it('counts and pushes for a loaded player', () => {
      ...
      netHandlers['gphone:server:shell:checkPhoneItem']();
