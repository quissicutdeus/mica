...
/**
 * MICA-151: micaOS with no framework resource at all.
 *
...
describe('the standalone player', () => {
  ...
  it('refuses an over-length identifier rather than truncating it', () => {
    ...
    // self-consistent inside micaOS and joins to nothing outside it, which is what makes the
...
describe('the qb shape everything downstream reads', () => {
  ...
  it('carries the number micaOS issued, on the player and in the qb view', () => {
...
describe('standalone metadata degrades and says so', () => {
  it('drops the write without throwing', () => {
    ...
    expect(() => player?.setMeta('mica_battery', 42)).not.toThrow();
  });
  ...
  it('reports it once per resource start, because it is a property of the server', () => {
    ...
    for (let i = 0; i < 10; i++) player?.setMeta('mica_battery', i);
    other?.setMeta('mica_battery', 1);
  ...
  it('names the key that was dropped, so the log is actionable', () => {
    FrameworkBridge.getPlayer(5)?.setMeta('mica_battery', 42);

    expect(warnings()[0]).toContain('mica_battery');
  });
});
...
describe('the owner table, which decides what the orphan sweep may delete', () => {
  it('is null on standalone, so the sweep skips', () => {
    ...
    // here — a leftover `players` from a previous install, say — reads every micaOS row as
...
describe('offline lookups on standalone', () => {
  it('never queries the `players` table, which does not exist here', async () => {
    ...
    expect(queried.every((query) => query.includes('mica_phone_numbers'))).toBe(true);
  });

  it('renders an offline player as the number micaOS issued them', async () => {
    // micaOS is the only record a standalone player has, so its own table is the framework
