...
// Imported for its side effect: loading the module registers `gphone:server:messages:get`.
...
/** Drives `gphone:server:messages:get` as the caller, a participant unless told otherwise. */
const get = async (data: unknown, { member = true } = {}) => {
  const handler = handlers.get('gphone:server:messages:get');
...
describe('a thread page is three statements at most, whatever the thread holds', () => {
  ...
  it('spends them on membership, the page and one IN for its attachments', async () => {
    ...
    expect(db.statements[0].sql).toContain('gphone_messages_participants');
    expect(db.statements[1].sql).toContain('FROM gphone_messages m');
    ...
    expect(db.statements[2].sql).toContain('FROM gphone_messages_attachments a');
  ...
  it('reads nothing at all for someone who is not in the thread', async () => {
    ...
    expect(db.count(/FROM gphone_messages m/)).toBe(0);
  });
});
...
describe('the page is a keyset on id, newest first, in reading order', () => {
  it('asks for one row more than the page and drops it, keeping its id as the cursor', async () => {
    ...
    const page = statement(/FROM gphone_messages m/);
  ...
  it('clamps an oversized limit to the declared maximum rather than refusing it', async () => {
    ...
    expect(statement(/FROM gphone_messages m/).params).toEqual([7, MAX_PAGE + 1]);
  });
  ...
  it('never orders by created_at — an insert at the head must not shift the keyset', async () => {
    ...
    expect(statement(/FROM gphone_messages m/).sql).not.toContain('ORDER BY m.created_at');
  });
});
