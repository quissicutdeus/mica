...
const CHARACTER_DELETED_EVENT = 'gphone:server:shell:characterDeleted';
...
/**
 * A server where everything is answerable: characters exist, gPhone has rows, and the
...
describe('which tables the sweep covers, and how that set is derived', () => {
   ...
   *
   * `gphone.sql` is the committed artifact an operator imports, so holding the swept set
   ...
   * `gphone_audit_logs` — the one table with no `defineService` behind it — is kept honest
   * rather than trusted to a comment.
   */
  it('covers exactly the tables in the committed gphone.sql that carry a citizenid', () => {
    const sql = readFileSync(join(__dirname, '..', '..', 'gphone.sql'), 'utf8');
  ...
  it('leaves the child tables that cascade off gPhone’s own rows alone', () => {
    const tables = ownedTables().map((t) => t.table);
    // These key on `gphone_accounts(id)` / `gphone_blabber(id)`, whose foreign keys survive
  ...
  it('names the audit ledger, which no declaration produces', () => {
    expect(ownedTables().map((t) => t.table)).toContain('gphone_audit_logs');
  });

  it('keys on the citizenid column itself, never on a citizenid-shaped one', () => {
    // `gphone_reports.target_author` is a varchar(50) holding a citizenid with no foreign
...
describe('fail-closed: nothing is deleted on evidence the sweep does not have', () => {
   ...
   *
   * FiveM starts resources in `server.cfg` order and `ensure gphone` above
  ...
  it('skips when the owner table cannot be read at all', async () => {
    dbMock.single.mockRejectedValue(new Error('Table gphone.players doesn’t exist'));
  ...
  it('skips when it cannot sample gPhone’s own rows', async () => {
  ...
  it('does nothing when gPhone holds no rows to sweep', async () => {
    dbMock.single.mockResolvedValue({ total: 120 });
...
describe('the statement itself', () => {
  it('is the statement MICA-71 shipped, on qb, plus a bound on how much it may lock', () => {
    const sql = orphanDeleteSql({ table: 'gphone_media', column: 'citizenid' }, QB);
  ...
  it('asks the ESX question of the ESX table', () => {
    const sql = orphanDeleteSql({ table: 'gphone_notes', column: 'citizenid' }, ESX);

    expect(sql).toContain('FROM users p');
    expect(sql).toContain('p.identifier = gphone_notes.citizenid');
  });
  ...
  it('never uses NOT IN', () => {
    ...
    expect(orphanDeleteSql({ table: 'gphone_notes', column: 'citizenid' }, QB)).not.toContain(
      'NOT IN'
    );
  });
  ...
  it('refuses to build SQL from anything that is not a plain identifier', () => {
    ...
    expect(() =>
      orphanDeleteSql({ table: 'gphone_notes; DROP TABLE players; --', column: 'citizenid' }, QB)
    ).toThrow(/plain identifier/);
    expect(() => orphanDeleteSql({ table: 'gphone_notes', column: '*' }, QB)).toThrow(
      /plain identifier/
    );
    expect(() =>
      orphanDeleteSql({ table: 'gphone_notes', column: 'citizenid' }, { table: 'a b', column: 'c' })
    ).toThrow(/plain identifier/);
  });
});

describe('deleting in chunks rather than in one long lock', () => {
  const oneTable: OwnedTable[] = [{ table: 'gphone_media', column: 'citizenid' }];
...
describe('purging one named character', () => {
  ...
  it('deletes that citizenid’s rows from every owned table, bound as a parameter', async () => {
    ...
    for (const call of dbMock.query.mock.calls) {
      expect(String(call[0])).toMatch(/^DELETE FROM gphone_[a-z_]+ WHERE citizenid = \?$/);
...
describe('the character-deleted hook', () => {
  ...
  it('leaves the older media-only hook registered and media-only', async () => {
    ...
    expect(localHandlers.has('gphone:server:media:characterDeleted')).toBe(true);

    dbMock.query.mockResolvedValue({ affectedRows: 1 });
    localHandlers.get('gphone:server:media:characterDeleted')![0]('CID_Z');
    await new Promise((resolve) => setTimeout(resolve, 0));

    expect(deletes()).toEqual(['DELETE FROM gphone_media WHERE citizenid = ?']);
  });
});
