...
describe('0001_repair_conversation_participants', () => {
  ...
  it('removes duplicate participant rows before it counts anybody', async () => {
     ...
     * another.
     */
    expect(dedup).toContain('DELETE p FROM gphone_messages_participants');
    expect(recount).toContain('UPDATE gphone_messages_conversations');
  ...
  it('considers closed rows too, not only live ones', async () => {
    ...
    expect(grouping).toContain('FROM gphone_messages_participants GROUP BY');
...
describe('0001_repair_conversation_participants — the unique key', () => {
   ...
   * index at all — slower than before the upgrade, still unprotected, and `gphoneschema
  ...
  it('adds the unique key before dropping the old index', async () => {
    ...
    expect(ddl()).toEqual([
      'ALTER TABLE `gphone_messages_participants` ADD UNIQUE KEY `conversation_participant_unique` (`conversation_id`, `citizenid`)',
      'ALTER TABLE `gphone_messages_participants` DROP INDEX `conversation_participant`'
    ]);
  });
  ...
  it('asks information_schema before either DDL step, scoped to the current database', async () => {
    ...
    for (const [sql, params] of dbMock.scalar.mock.calls) {
      ...
      expect(params[0]).toBe('gphone_messages_participants');
    }
  });
   ...
   * A blind `ADD UNIQUE KEY` would error there and abort the rest of `gphoneschema apply`.
   */
