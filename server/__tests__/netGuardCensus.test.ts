 ...
 * micaOS's entry points starts. It has now been wrong twice: once when it said eight handlers
...
/** micaOS's own namespace. Anything else is named by a framework, which is the whole point. */
const isGphoneNamed = (event: string) => event.startsWith('mica:');
...
describe('the onNet census in netGuard.ts is true', () => {
  ...
  it('finds the number of mica-named handlers the docblock claims', () => {
    const stated = statedNumber(/(\w+) are mica-named/);
    expect(handlers.filter((h) => isGphoneNamed(h.event))).toHaveLength(stated);
  });

  it('finds them in exactly the files the docblock names', () => {
    // The prose lists the mica-named files inline. A handler in a fifth file is an entry
    ...
    const sentence = docblock.match(/are mica-named, across ([\s\S]*?`)\./);
    expect(sentence, 'netGuard.ts no longer lists the mica-named files').not.toBeNull();
 ...
 * says the census "used to read six" and that "three mica-named handlers had been added
...
describe('the onNet census in docs/security.md is true', () => {
  const handlers = registrations();
  const micaNamed = handlers.filter((h) => isGphoneNamed(h.event));
  ...
  const MICA_HEADING = /^#### mica-named — (\w+)/m;
  ...
  it('heads each category with the count the tree has', () => {
    expect(inDoc(MICA_HEADING)).toBe(micaNamed.length);
  ...
  it('agrees with netGuard.ts, so the two copies cannot drift apart', () => {
    ...
    expect(inDoc(MICA_HEADING)).toBe(statedNumber(/(\w+) are mica-named/));
  });
  ...
  it('reserves the four shapes, and not the words around them', () => {
    ...
    const narrative = [
      ...
      'Two mica-named handlers had been added since that paragraph was written.',
