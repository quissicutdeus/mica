...
describe('places — the declaration', () => {
  it('disables the generic create in favour of the custom one', () => {
    expect(handlers.has('mica:server:places:create')).toBe(true);
  ...
  it('registers the generic get/update/delete, owner-scoped like every other saved row', () => {
    expect(handlers.has('mica:server:places:get')).toBe(true);
    expect(handlers.has('mica:server:places:update')).toBe(true);
    expect(handlers.has('mica:server:places:delete')).toBe(true);
  });
});
