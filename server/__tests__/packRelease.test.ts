 ...
 *
 * `release.yml` runs on `main` and nowhere else, so the first time it saw `@gphone/sdk`
 * depend on `@gphone/shared` through `workspace:*` was the release itself, which failed with
...
describe('the manifest a release tarball ships', () => {
  ...
  it('pins a workspace dependency to the very version being published', () => {
    ...
    expect(packed.dependencies['@gphone/shared']).toBe(VERSION);
  });
    ...
    const packed = JSON.parse(
      releaseManifest(
        JSON.stringify({
          name: '@gphone/sdk',
          version: '1.0.0',
          dependencies: { '@gphone/shared': 'workspace:*', marked: '^18.0.11' },
