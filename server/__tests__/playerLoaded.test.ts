...
const { dbMock, bridgeMock, handlers, networked, framework } = vi.hoisted(() => {
  ...
  // property here: `onNet` declares the name net-safe inside micaOS and makes it reachable by
...
describe('the network player-loaded listeners', () => {
  ...
  it('still serves qbx_core, which sends no payload at all', async () => {
    ...
    expect(emitted()).toContainEqual(['mica:client:shell:rehydrate', ATTACKER]);
    expect(emitted()).toContainEqual(['mica:client:settings:rehydrate', ATTACKER]);
    // Battery's load is async — the row read happens before the push.
    await vi.waitFor(() =>
      expect(emitted()).toContainEqual(['mica:client:battery:set', ATTACKER, 100])
    );
  });
 ...
 * micaOS would never rehydrate settings and never load battery, with no output anywhere and
 ...
 * no way in for a packet to be refused. micaOS registers this name with `on` only, and
 * `RegisterNetEvent`'s net-safety flag is per-resource, so the name is not net-safe inside
 * micaOS and a client emitting it reaches nothing. The first test below is the one that
...
describe('the player-loaded registry', () => {
  ...
  it('keeps running the others when one subscriber throws', async () => {
    // A broken subscriber degrades rather than taking the character load with it — micaOS has
...
describe("ESX's player-loaded event", () => {
  ...
  it('rehydrates the shell for the player id ESX names', () => {
    ...
    expect(emitted()).toContainEqual(['mica:client:shell:rehydrate', VICTIM]);
  });
