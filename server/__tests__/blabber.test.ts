...
const call = async (action: string, data: unknown, citizenid = 'CIT_A') => {
  const handler = handlers.get(`mica:server:blabber:${action}`);
...
describe('the declaration', () => {
  ...
  it('makes an ear unique per account, in the child table', () => {
    const ears = blabber.resolved.childTables.find((t) => t.name === 'mica_blabber_ears');
...
describe('attachments', () => {
  it('allows a picture post with no body, as long as it owns the photo', async () => {
    ...
    expect(String(table)).toContain('mica_blabber_attachments');
...
describe('hashtag indexing', () => {
  it('writes one row per distinct tag on create', async () => {
    ...
    const tagInserts = dbMock.insert.mock.calls.filter(([sql]) =>
      String(sql).includes('mica_blabber_tags')
    );
...
describe('the public feed', () => {
  it('answers unfiltered when no account_id is supplied', async () => {
    ...
    const [sql, params] = dbMock.query.mock.calls[0];
    expect(sql).not.toContain('mica_account_blocks');
 ...
 * `avatar` are rendered by `BlabRow`, `Thread` and `Profile`, nothing joined `mica_accounts`,
...
describe('author hydration', () => {
  ...
  it('never selects the author citizenid', async () => {
    ...
    const sql = String(dbMock.query.mock.calls[0][0]);
    expect(sql).toContain('mica_accounts');
    expect(sql).not.toContain('citizenid');
  });
