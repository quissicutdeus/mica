...
/**
 * MICA-157. `gos.sql` declares every table `COLLATE = utf8mb4_unicode_ci` (see
 * `schemaSql.ts`'s `TABLE_COLLATION`), and 22 of its 29 tables carry a foreign key onto
 * `players(citizenid)` — a table the framework owns, not gOS. A foreign key requires both
 ...
 * modern MariaDB mismatches gOS's explicit one, and the raw SQL import fails partway
 ...
 * time an operator is in a position to ask this resource anything at all — `gosschema
 ...
 * no `players` table (`schemaSql.ts`'s `OWNER_TABLE` docs), and `gos.esx.sql` carries no
...
/**
 * The collation an already-created gOS table's own `citizenid` column actually carries,
 ...
 * back to `TABLE_COLLATION` (the caller's job) when no gOS table has been created yet at
...
/**
 * Compare `players.citizenid`'s live collation against what gOS's own tables use, before
...
export const collationMismatchMessage = (mismatch: CollationMismatch): string =>
  `[gosschema] refusing to apply: \`${mismatch.ownerTable}\`.\`${mismatch.ownerColumn}\` is ` +
  `collated ${mismatch.ownerCollation}, but gOS's own tables use ${mismatch.expectedCollation}. ` +
