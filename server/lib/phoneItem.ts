 ...
 * buy, lose or have taken. `gphone_phone_item` names an inventory item; while it is set, the
 ...
 * inventory to hold the item in. `gphone_standalone` with `gphone_phone_item` set is reported
...
export const PHONE_ITEM_CONVAR = 'gphone_phone_item';

/** An inventory item name: what qb, ox_inventory and ESX all accept, and nothing else. */
const ITEM_NAME = /^[A-Za-z0-9_-]{1,64}$/;

const PUSH_EVENT = 'gphone:client:shell:phoneItem';
const OPEN_EVENT = 'gphone:client:shell:open';
...
export const phoneItemName = (): string | null => {
  ...
  if (!ITEM_NAME.test(raw)) {
    reportOnce(
      'name',
      `[gphone] ${PHONE_ITEM_CONVAR} is set to '${raw}', which is not an item name any ` +
  ...
  if (detectFramework() === 'standalone') {
    reportOnce(
      'standalone',
      `[gphone] ${PHONE_ITEM_CONVAR} is set to '${raw}', but this server runs standalone and ` +
...
const heldBy = (player: FrameworkPlayer, item: string): boolean => {
  ...
  if (count === null) {
    reportOnce(
      'count',
      `[gphone] ${PHONE_ITEM_CONVAR} is '${item}', but no inventory here can say how many a ` +
 ...
 */
onNet('gphone:server:shell:checkPhoneItem', () => {
  const src = source;
