 ...
 * and aborts the rest of `gphoneschema apply`. `table_schema = DATABASE()` confines the
 ...
 * rows it forbids.
 *
 * **This migration deletes rows**, and outside `gphonemedia prune` it is the only thing in
...
export const migration: Migration = {
  ...
  description:
    'deletes duplicate participant rows, recomputes gphone_messages_conversations.is_group from the live participant count, and replaces the conversation_participant index with a unique one',
  up: async () => {
    /**
     ...
     */
    await Database.query(
      `UPDATE gphone_messages_conversations c
         JOIN (
     ...
     * and `gphoneschema apply` aborted. Adding first means a failure leaves the table
