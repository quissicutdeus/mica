...
/**
 * Widen `gphone_audit_logs.action` to accept `'viewed'` (MICA-70): an admin reading
 * reported content, rather than acting on it.
 *
 * `gphone_audit_logs` has no `defineService` behind it — it is emitted verbatim from
...
export const migration: Migration = {
  ...
  description:
    "widens gphone_audit_logs.action to accept 'viewed' — an admin reading reported " +
    'content rather than acting on it',
  up: async () => {
    await Database.query(
      `ALTER TABLE \`gphone_audit_logs\` MODIFY COLUMN \`action\` ENUM(
         'archived',
         'unarchived',
         'deleted',
         'left',
