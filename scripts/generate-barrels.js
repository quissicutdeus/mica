...
const generateIndex = (dir, mode = 'import', exclude = []) => {
  ...
  // `@mica/sdk` and pulled vitest into the shipped bundle. The same applies to the
  ...
  // through `@mica/sdk`). Named by module basename, no extension.
  const files = fs
    .readdirSync(fullDir)
    .filter(
      (file) =>
...
// Do not edit this file manually.
//
// Apps import icons from '@mica/sdk', never by relative path: an add-on installed
