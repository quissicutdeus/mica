...
#   no-agent-forwarding,no-pty ssh-ed25519 AAAA... gos-ci-deploy-dev
#
...
cd "/opt/fivem-dev/server-data/resources/[standalone]/gos/"

git fetch https://github.com/quissicutdeus/gos.git dev
...
sudo GOS_PORT=8676 GIT_BRANCH=dev GIT_SHA="$GIT_SHA" GOS_CALVER="$GOS_CALVER" GOS_CONTAINER_NAME=gos-dev GOS_IMAGE_TAG=gos-dev:local \
    /usr/local/sbin/gos-deploy-dev-compose.sh
