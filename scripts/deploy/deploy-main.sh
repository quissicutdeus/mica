...
cd "/opt/fivem-main/server-data/resources/[standalone]/mica/"

git fetch https://github.com/quissicutdeus/mica.git main
...
install -m 755 scripts/deploy/deploy-main.sh "$HOME/bin/.deploy-main.sh.new"
mv -f "$HOME/bin/.deploy-main.sh.new" "$HOME/bin/deploy-main.sh"
...
sudo MICA_PORT=8675 GIT_BRANCH=main GIT_SHA="$GIT_SHA" MICA_CALVER="$MICA_CALVER" MICA_CONTAINER_NAME=mica-main MICA_IMAGE_TAG=mica-main:local \
