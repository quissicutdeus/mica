...
#
# The RCON reload lives HERE, not in ~gphone/bin/deploy-dev.sh, because the password
# is in /opt/fivem-dev/.env -- 0600, owned by mbiddle. gphone cannot read it; root
...
COMPOSE_FILE="/opt/fivem-dev/server-data/vendor/mica/compose.yaml"
...
docker compose -p mica-dev -f "$COMPOSE_FILE" up -d --build
...
if [[ -z "$RCON_PASSWORD" ]]; then
    echo "REFUSED: no RCON_PASSWORD in $ENV_FILE -- micaOS was rebuilt but NOT reloaded" >&2
    exit 1
fi
