 ...
 * `resources/`. Lowercase, matching README's `ensure mica` and the `mica:` event prefix.
 */
export const RESOURCE_NAME = 'mica';
...
export const TOP_LEVEL_FILES = ['fxmanifest.lua', 'mica.sql', 'mica.esx.sql', 'LICENSE'];
 ...
 * it is, so `ensure mica` and `resources` in a server console name a real version.
...
export function readmeExcerpt(readme, { tag, repository }) {
  ...
  return (
    `# micaOS ${version}\n\n` +
    "This is the prebuilt release of micaOS, ready to unpack into a FiveM server's\n" +
