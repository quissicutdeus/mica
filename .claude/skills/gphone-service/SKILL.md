...
description:
  ...
  resource it does not own. Covers gphone.sql generation and gphoneschema apply.
---
...
- `table` overrides the default `gphone_<id>`; `options` (`disableGet`,
...
`gphone.sql` is generated in full, committed, and **imported by hand**. Never
...
A live install is brought up to date by **`gphoneschema apply`** from the server
...
export const migration: Migration = {
  ...
  up: async () => {
    await Database.query('ALTER TABLE `gphone_media` CHANGE COLUMN ...', []);
  }
};
```
...
behind a `*Bridge` in `server/lib/`. Querying their tables couples gPhone to a
