...
RUN --mount=type=cache,id=mica-pnpm-store,target=/pnpm/store,sharing=locked \
...
RUN --mount=type=cache,id=mica-go-build,target=/root/.cache/go-build \
    CGO_ENABLED=0 go build -trimpath -ldflags='-s -w' -o /out/mica-serve . \
 && if ldd /out/mica-serve 2>/dev/null | grep -q '=>'; then \
      echo 'mica-serve is dynamically linked; it cannot run in scratch' >&2; \
      exit 1; \
    fi
...
RUN if [ "$COMPRESS_BINARY" = "1" ]; then \
      ...
      upx -q --best /out/mica-serve >/dev/null && \
      upx -qt /out/mica-serve >/dev/null; \
    fi
...
LABEL org.opencontainers.image.source="https://github.com/quissicutdeus/mica"
...
HEALTHCHECK --interval=30s --timeout=3s --start-period=2s --retries=3 \
  CMD ["/mica-serve", "-health"]

ENTRYPOINT ["/mica-serve"]
