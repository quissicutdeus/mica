...
What changed for the people who run gOS on a server, and what they have to do
...
changes that put `gosschema apply` in front of an owner lands without being
...
different things: an owner runs `gosschema apply`, an author rebuilds and
...
**The resource is renamed from `gPhone` to `gos`, and everything keyed on the
old name moves with it (MICA-269).** Rename the resource folder to `gos`,
change your `ensure` line to match, and rename any `gphone_*` convar, any
`gphone.admin` ace, and any `exports['gphone']` call in your own scripts to the
`gos` spelling. A convar you miss does not error — it silently falls back to its
default, and an ace you miss silently stops granting the admin tools, so this is
worth a read of your `server.cfg` rather than a glance.

**Your database moves with it, and migration `0004_rename_gphone_tables_to_gos`
does that for you — run `gosschema apply` from your server console after
updating, or import the regenerated `gos.sql` / `gos.esx.sql` on a fresh
install.** Every `gphone_*` table is renamed to `gos_*` in one atomic statement,
so no foreign key ever sees a half-renamed schema. If you start the new resource
_before_ running the migration, the additive half of `gosschema apply` will
create the new tables empty beside your old ones and every player will look like
a fresh install; the migration then refuses to overwrite the empty table and
tells you which pair to look at, so nothing is lost, but the tidy order is to
apply first.
...
both run gOS. This release is **Seraphim**, the first of the nine choirs the
codenames now follow.

**`gos_messages` gains a `conversation_id_id` key on `(conversation_id, id)` —
run `gosschema apply` from your server console after updating, or import the
regenerated `gos.sql` / `gos.esx.sql` on a fresh install (MICA-218).** A
...
**`gos_messages` gains a `reply_to_id` column and a `reply_to_id` key — run
`gosschema apply` from your server console after updating, or import the
regenerated `gos.sql` / `gos.esx.sql` on a fresh install (MICA-209).** Until
...
**`gos_phone_numbers` is a new table — run `gosschema apply` from your server
console after updating, or import the regenerated `gos.sql` / `gos.esx.sql` on a
fresh install.** It carries `id`, `citizenid`, `number`, `status`, `created_at`
and `updated_at`, with a `status` key, a `citizenid_status` key, and two unique
keys, `number_unique` and `citizenid_unique`. It is how gOS issues a phone
number on a server running with no framework, where there is no framework to
issue one — a number is generated at random on a player's first connection and
stays with them across reconnects. **On qb and ESX the table is created and
stays empty**, and the framework's own number is used exactly as before; nothing
about an existing server's numbering changes. It is created on every framework
rather than only on standalone because `gos.sql` and `gos.esx.sql` differ only
in their foreign keys onto `players`, and a third artifact would be a third
thing to import the wrong one of.

**`gos_contacts` gained a `ringtone` column — run `gosschema apply` from your
server console after updating, or import the regenerated `gos.sql` /
`gos.esx.sql` on a fresh install.** Contacts can now carry a per-contact
...
**`gos_audit_logs.action` gained a `viewed` value, via a new migration
(`0002_audit_logs_add_viewed_action`) — run `gosschema apply` from your server
console after updating, or import the regenerated `gos.sql` / `gos.esx.sql` on a
fresh install.** An admin opening the report queue or history now writes an
...
**`gos_messages_conversations` gained `participant_a`, `participant_b` and a
generated `pair_key` column, plus a migration (`0003_conversations_pair_key`) —
run `gosschema apply` from your server console after updating, or import the
regenerated `gos.sql` / `gos.esx.sql` on a fresh install.** A 1:1 thread's two
...
**`gos_lockscreen` is a new table — run `gosschema apply` from your server
console after updating, or import the regenerated `gos.sql` / `gos.esx.sql` on a
fresh install.** The lock screen's passcode (display state, not a security
...
`passcode_salt`, plus the usual `status` and `updated_at` every gOS table
...
**`gos_places` is a new table — run `gosschema apply` from your server console
after updating, or import the regenerated `gos.sql` / `gos.esx.sql` on a fresh
...
your economy, controlled by a new convar, `gos_hodlr_spread_pct` (default `2`,
...
it still uses the flat mid price. Set `gos_hodlr_spread_pct 0` to keep the old
single-price behavior exactly. This ships with no cooldown and no per-trade
position limit beyond the existing `gos_hodlr_trade_max` — the spread itself is
what discourages rapid trading, by construction.

**`gos_blocklist` is a new table — run `gosschema apply` from your server
console after updating, or import the regenerated `gos.sql` / `gos.esx.sql` on a
fresh install.** A player can now block a phone number; a blocked call fails
...
`citizenid_number_unique`. A new convar, `gos_emergency_number` (default `911`),
always connects regardless of any block — a player registered under that phone
number (a dispatch resource's own setup, not a new gOS feature) is reachable no
matter who dials it. `GetEmergencyNumber` is a new export for that resource to
read the configured number rather than duplicate the convar name.

**`gos_messages_reactions` is a new table — run `gosschema apply` from your
server console after updating, or import the regenerated `gos.sql` /
`gos.esx.sql` on a fresh install.** Native Messages (SMS-style threads) can now
...
`gos_account_reactions`: that table's reactor is an `account_id`, which Messages
has no equivalent of, so this one keys a reaction on `message_id`, `citizenid`,
`emoji` and `created_at` instead, alongside the usual `id`. No existing table
changed shape.
...
for `[FrameworkBridge]` errors after this update.** gOS now refuses a money move
its framework did not confirm with a plain `true`, and refuses to spend against
a balance that did not come back as a number — see the first entry under Fixed.
On a framework whose money calls answer the way they always have, nothing
...
**If you run ESX, a player has one phone — not one per character.** gOS keys
...
phone on, and giving gOS a second notion of identity would have put the question
into every ownership check in the resource instead of one place (MICA-150).

**Moving an existing server between ESX and a qb core re-keys every phone, and
gOS ships no migration for it.** The rows written under the old identity stay in
the tables, but nobody resolves to them any more, so players arrive to empty
...
**Run `gosschema apply` from your server console after updating.** This release
carries the first versioned migration gOS has ever shipped,
...
**This migration deletes rows, and it is the only part of gOS outside
`gosmedia prune` that does.** It removes the surplus participant rows — the ones
naming a person a second, or five-hundredth, time in a thread they were already
in — keeping one row for each person in each conversation. Those rows are
artefacts of the bug and nobody asked for them, but you are entitled to know
...
again. On `gos_messages_participants`, the index `conversation_participant` is
...
than a convention. The rename is not cosmetic: gOS compares the indexes on your
database against the ones it expects **by name only**, so had the existing name
been reused your server would have kept a non-unique index forever and reported
nothing wrong. The migration therefore adds and drops the index itself rather
than trusting that comparison, and both steps check whether they are needed
first — so an apply that is interrupted can simply be run again. The new key
goes on before the old one comes off, so if anything does go wrong your table is
left exactly as it was rather than with no index at all.
...
**The phone can be an item (MICA-229).** Set `gos_phone_item` to the name of
...
`gos-<version>.zip` (MICA-220).** Unpack it into `resources` and `ensure gos`:
...
its own `gos.esx.sql` and starts it; a zip that does not start is not released.
...
`gos_locale`, then the player's own game language, then English. Set
`gos_locale "de"` (a BCP 47 tag; a value that is not one is ignored with a
...
text as the admin typed it (`goscall`, `gosseed` and the battery command echo),
since it is not fixed prose. No owner action.
...
- The lock screen passcode is stored with **scrypt** rather than a single salted
  ...
  only in the UI. Two new convars come with it: `gos_lockscreen_scrypt_cost`
  (default `16384`, and it must be a power of two) tunes the hashing cost for
  operators on weak hardware, and `gos_lockscreen_max_attempts` (default `5`)
...
- gOS now runs **standalone**, with no framework resource at all, when you set
  the new `gos_standalone` convar. Identity comes from the player's FiveM
  ...
  as a misconfiguration — the real framework wins and gOS says so once in the
  console. Standalone servers import `gos.esx.sql`, get gOS-issued phone
...
- **A deleted Contact, Note or photo can be restored again**, within a shared
  window (`gos_restore_window_days`, default 30 days) after the delete.
  ...
  nothing in gOS hard-deletes a contact, note or photo, since the moderation
  system depends on a soft-deleted row surviving forever.
- **gOS runs on ESX.** `es_extended` joins `qbx_core` and `qb-core` as a
  ...
  Money is handled the way the rest of gOS handles it — refuse what cannot be
  ...
  answer to judge the way a qb core's `true` is judged. gOS instead reads the
  ...
  **Import `gos.esx.sql`, not `gos.sql`.** There are now two schema files, both
  generated and both creating the same twenty-nine tables. The ESX one carries
  none of the foreign keys onto `players`, because ESX has no such table — it
  keeps players in `users`, by `identifier`. Importing the wrong file fails at
  the first foreign key rather than half-working.
  ...
  `gos:server:media:characterDeleted` is not optional there the way it is on a
  qb core. Phone numbers are not part of core ESX, so a player whose number
  lives somewhere gOS does not look will have no number and no number-based
  lookup — and an **offline** player is found by identifier but never by number,
  core ESX having nowhere to keep one. And the battery level gOS mirrors onto
...
- Proximity music, which lets a phone play out loud to the people standing
  around it, is bounded by two new convars: `gos_music_range` for how far it
  carries, and `gos_music_max_nearby` for how many broadcasters one listener is
  told about at once (MICA-111).
- `gos_camera_quality` sets how hard a photo is squeezed before it is stored,
...
- A Bluetooth proximity share now reaches at most five phones, nearest first,
  set by the new `gos_bluetooth_max_nearby` convar. It was previously everyone
...
- A photo larger than 4MB is refused rather than stored. The column holds 16MB
  ...
  `gos_media` far faster than any camera can. Nothing the phone's own camera
  produces comes close to the cap — `gos_camera_quality` is still the knob for
  how big stored photos actually get (MICA-116).
- Each player's photo library now has a ceiling, set by the new
  `gos_media_quota_mb` convar and defaulting to 64MiB — roughly 150 to 200
...
- `gos_media_retention` deletes stored media older than a number of days you
  ...
  `gosmedia prune`, which is console-only for the same reason `gosschema apply`
  is. `gosmedia` with no argument still only reports (MICA-71).
- A deleted character's photos are cleaned up rather than left behind. The table
  ...
  `gos:server:media:characterDeleted` with a citizenid to reclaim the space at
  once (MICA-71).

- `gos_hodlr_trade_max` caps what a single Hodlr buy or sell can be worth,
  defaulting to 50,000 — the same number and the same shape as
  `gos_bank_transfer_max`, which was until now the only value cap on the phone.
...
different place to put it. The day gOS stores real video that answer changes,
...
- Add-on bundles carried a fabricated version. `vite.addon.config.ts` had no
  `define` block, so `__GOS_VERSION__` was never substituted and every add-on's
  `GOS_VERSION` fell through to a hard-coded `1.0.0` — on every server, forever,
  while the phone's own bundle carried the real CalVer. The one version number
  an add-on could read was wrong in exactly the bundles that needed it, and it
  was wrong in the shape that reads as success rather than as an error. **If you
  author an add-on, note the new value is the empty string, not the real
  version**: a bundle is compiled once and then installed by whatever phone
  ...
  `if (GOS_VERSION)` before using it. (MICA-170)
- `@gos/sdk` now exports `SDK_CONTRACT_VERSION`, which is the number an add-on
...
- **A framework that stops answering money calls the way it used to can no
  longer create currency.** gOS asks your framework to debit and credit players,
  and it believed whatever came back. If a qb-core or qbx_core release made
  `RemoveMoney` asynchronous — an ordinary thing for a resource to do — the
  ...
  and the spend went through. gOS pins the FiveM runtime exactly and cannot pin
  your framework, so this was one upgrade away on any server, with no attacker
  involved and no error to see — the first sign would have been an economy that
  no longer balanced. Every one of those branches now refuses instead: only a
  literal `true` counts as money having moved, only a finite number counts as a
  balance, and anything else is logged as a `[FrameworkBridge]` error naming the
  call that answered oddly. Bank transfers and Hodlr trades are the two places
  you would notice (MICA-133).
...
- **Hodlr refuses to guess a price it cannot recover.** If the price history is
  ...
  `gos_hodlr_price_history` from a backup reopens it with no restart. The hourly
  pruning sweep also now keeps the newest row whatever its age — it is the
  price, not a chart point, and a server that had been down longer than the
...
- **A database that stops answering mid-restore no longer wedges the market
  ...
  — every trade refused, chart flat, only `restart gos` to recover. An
...
`@gos/sdk`, and a catalog entry carries `devices` the same way. No hook was
...
checks a download;
`gh attestation verify gos-sdk-<version>.tgz --repo quissicutdeus/gos` proves
...
`postMessage` instead of through `@gos/sdk` is refused. Each frame is also
...
from `@gos/shared/contract` and pass it to `defineService` as `contract`.
...
bundle, maps each `@gos/sdk` import through the permission table, and stops the
...
exported from `@gos/sdk`, and it is the number to branch on. It moves when the
...
`GOS_VERSION` is not that number: it is the running phone's CalVer build stamp,
...
`LICENSE_SOURCE_OFFER`, `GOS_SOURCE_URL` and `sourceUrlForBuild` are exported
beside it, along with `GOS_BRANCH` (the empty string inside an add-on bundle,
like `GOS_VERSION`). Nothing was removed to make room for any of it.
...
date has required `gosschema apply` for a rename, a retype or a drop.

### Added

- Every convar gOS reads is now documented in the README's
...
- The schema is [`gos.sql`](gos.sql), imported by hand. Every statement is
  `CREATE TABLE IF NOT EXISTS`, so re-importing an existing database is
  harmless.
- gOS applies no schema change on its own. `gosschema` in the server console
  reports where the database differs from the code and changes nothing;
  `gosschema apply` — console only — applies pending versioned migrations and
  then the safe additive half of the difference.
- When an entry above says a migration landed, run `gosschema apply` from the
