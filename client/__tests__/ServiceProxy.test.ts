...
describe('shared/rpc — one derivation for both sides', () => {
  it.each([
    ...
    expect(responseEventFor('notes', action)).toBe(`mica:client:notes:${reply}`);
  });

  it('replies to a custom action on its own name', () => {
    expect(responseEventFor('mail', 'getMail')).toBe('mica:client:mail:getMail');
  });

  it('builds request events', () => {
    expect(requestEventFor('notes', 'get')).toBe('mica:server:notes:get');
  });

  it('round-trips a request event back to service and action', () => {
    expect(parseRequestEvent('mica:server:mail:markAsRead')).toEqual({
      service: 'mail',
      action: 'markAsRead'
    });
  });
  ...
  it.each([
    ['too few segments', 'mica:server:noServiceSegment'],
    ['wrong side', 'mica:client:notes:get'],
...
describe('ServiceProxy — request/response round trip', () => {
  it('resolves the NUI callback when the reply arrives', async () => {
    ...
    expect(event).toBe('mica:server:mail:getMail');

    // ...and the derived reply resolves that same id.
    netSubscriptions.get('mica:client:mail:getMail')!(cbId, [{ id: 1, subject: 'Statement' }]);
