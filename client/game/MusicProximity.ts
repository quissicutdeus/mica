 ...
 *
 * `gphone_music_range` — the same convar and the same {@link DEFAULT_MUSIC_RANGE} the
 * server fans out on, deliberately one number rather than two that can disagree. It is not
 * `gphone_bluetooth_range`, which is a hand-to-hand 15m and means something else.
 *
 * **A client only sees replicated convars.** `set gphone_music_range 40` in `server.cfg`
 ...
 * `gphone_music_max_audible` before it sends, and `parseNearbyMusicEnvelope` bounds it
