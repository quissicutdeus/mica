 ...
 * which is why the defaults below are `''` rather than a gPhone-operated catalog: there is
 ...
 * `gphone_camera_quality` and `gphone_music_range` are replicated (README).
 *
 ...
 */
const HOSTS_CONVAR = 'gphone_addon_hosts';
const CATALOG_CONVAR = 'gphone_addon_catalog';
...
export const remoteAppConfig = (): RemoteAppConfigPayload => {
  ...
  return {
    hosts: parseTrustedHosts(GetConvar('gphone_addon_hosts', '') ?? ''),
    catalogUrl: parseCatalogUrl(GetConvar('gphone_addon_catalog', '') ?? '')
  };
};
 ...
 * setting `gphone_addon_catalog` and forgetting `gphone_addon_hosts`: the Store then lists
 ...
 * awaits answers that question exactly. `gphone_camera_quality` is the existing precedent —
...
if (startupWarning) console.warn(`[gPhone] ${startupWarning}`);
