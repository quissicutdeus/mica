...
`author` — a display string — which meant naming yourself "gPhone" silently made
your app permanent.

`id` is the only name that matters. It is lower_snake_case and it is a **key**,
not a label: the directory name, the `gphone:journal:` storage namespace, the
...
something a third-party app cannot do.
...
gPhone, and your app does not. `useService` is the door that does not require
...
the file, and if that happens while the `@gphone/sdk` barrel is still
initialising you get `undefined` back — the symptom is
`byNewest is not a function` on a line that plainly imports it. Build it on
first use. A manifest is the usual trigger, because the registry loads every
manifest eagerly, so use
...
and naming yourself `'gPhone'` was enough to make one permanent. The derivation
...
add one** — `shared/routes.ts` ships inside gPhone, so a `core: false` app
...
`gphone:server:<id>:<action>`. Notes and Blabber are the two worked examples and
...
`web/src/services/` both ship inside gPhone, so an app installed from the Store
...
- `fetchNui` does not exist in the bundle. All host access goes through
  `@gphone/sdk` hooks, which route over `postMessage` to the shell; `useService`
