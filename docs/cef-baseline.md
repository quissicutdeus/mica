...
| 5   | M3 state layers composited numerically in JS instead of `color-mix()`  | `web/src/lib/m3.ts` (`composite`), consumed by `sdk/ui/NowPlayingCard.svelte`'s runtime cover tint (MICA-111) among others | Chrome 111                                                                              | Optional. Keeping it is defensible — one code path, no cascade recovery                                                    |
...
| WebP encoding, checked, falling back to JPEG                      | `web/src/lib/thumbnail.ts` (`encodeCanvas`)                                                                                                             | Chrome 32 — already safe. Moved out of `apps/camera/capture.ts`, which now only calls it — the gallery's own thumbnailing needed the same encoder       |
