...
| No `a` in the DOMPurify allowlist                                 | `sdk/lib/markdown.ts`                                                                                                                                 | Same, and a griefing vector (§7)                                                                                                                        |
| Resources served from `https://cfx-nui-<resource>/`               | —                                                                                                                                                     | A FiveM property                                                                                                                                        |
| Self-hosted `@fontsource/roboto`, not a Google Fonts `<link>`     | `sdk/app-reset.css`                                                                                                                                   | Needs a network request the client may not have. Not a version question                                                                                 |
...
| WebP encoding, checked, falling back to JPEG                      | `sdk/lib/thumbnail.ts` (`encodeCanvas`)                                                                                                               | Chrome 32 — already safe. Moved out of `apps/camera/capture.ts`, which now only calls it — the gallery's own thumbnailing needed the same encoder       |
...
MICA-67 frames video as a Chromium 103 limitation. Read from the code, it mostly
is not, and an upgrade will not on its own unblock it.
...
- `mica_media` already stores what playback would need: `url` (`varchar(512)`,
...
not a `<video>` element, and not `mica_media`, so nothing above changes it.
