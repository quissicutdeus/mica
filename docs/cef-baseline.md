...
| 5   | M3 state layers composited numerically in JS instead of `color-mix()`  | `web/src/lib/m3.ts` (`composite`)                                                     | Chrome 111                                                                              | Optional. Keeping it is defensible — one code path, no cascade recovery                                                    |
...
| Every `var()` in an inline `style=` must resolve              | `sdk/cef.test.ts`                              | MICA-85. An unresolvable `var()` invalidates the whole declaration silently. Permanent |
| `bg-transparent` on the app wrapper                           | `shell/Shell.svelte`                           | The game renders behind the overlay. A FiveM property                                    |
| No `window.location`, `window.open`, or anchor navigation     | AGENTS.md §6, `lib/markdown.ts`                | A redirect reloads the CEF instance and drops all state. A FiveM property                |
| No `a` in the DOMPurify allowlist                             | `web/src/lib/markdown.ts`                      | Same, and a griefing vector (§7)                                                         |
...
| WebP encoding for camera captures                             | `apps/camera/capture.ts`                       | Chrome 32 — already safe                                                                 |
