...
| 5   | M3 state layers composited numerically in JS instead of `color-mix()`  | `web/src/sdk/lib/m3.ts` (`composite`), consumed by `sdk/ui/NowPlayingCard.svelte`'s runtime cover tint (MICA-111) among others | Chrome 111                                                                              | Optional. Keeping it is defensible — one code path, no cascade recovery                                                    |
...
| 10  | `linear()` easing avoided; `--ease-emphasized` is one cubic-bezier     | `web/src/sdk/app.css` (`--ease-emphasized`)                                                                                      | Chrome 113                                                                              | Optional: restore M3's true two-segment emphasized curve                                                                   |
...
| No `a` in the DOMPurify allowlist                                 | `web/src/sdk/lib/markdown.ts`                                                                                                                           | Same, and a griefing vector (§7)                                                                                                                        |
| Resources served from `https://cfx-nui-<resource>/`               | —                                                                                                                                                       | A FiveM property                                                                                                                                        |
| Self-hosted `@fontsource/roboto`, not a Google Fonts `<link>`     | `web/src/sdk/app-reset.css`                                                                                                                             | Needs a network request the client may not have. Not a version question                                                                                 |
...
| WebP encoding, checked, falling back to JPEG                      | `web/src/sdk/lib/thumbnail.ts` (`encodeCanvas`)                                                                                                         | Chrome 32 — already safe. Moved out of `apps/camera/capture.ts`, which now only calls it — the gallery's own thumbnailing needed the same encoder       |
