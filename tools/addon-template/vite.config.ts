 ...
 *
 * This file is the standalone twin of micaOS's own `web/vite.addon.config.ts`, and it has
 ...
 *
 * micaOS's `web/src/lib/addonTemplate.test.ts` compares the two files knob for knob and
 * fails the phone's own build if they diverge, so the copy you are reading is checked
 * against the original rather than left to rot. That check runs in micaOS's repo, though,
...
/**
 * No alias for `@mica/sdk` — the package resolves to the right barrel on its own.
 *
 * This used to be the most important line in the file. `@mica/sdk` names two different
 ...
 * rather than writing an alias is the one outside micaOS's repo, which is you. The shell
 ...
 * mean and a bare `/core:\s*true/` over the raw file found the prose. micaOS's own
 ...
 * can compile a `.ts` file that imports Svelte components. micaOS's own add-on discovery
 * reads the same manifests the same way, for the same reason.
 */
function readManifest(): { id: string } {
  if (!fs.existsSync(MANIFEST)) {
    throw new Error(
      `[mica-addon] no manifest at ${MANIFEST} — an add-on is a manifest and a component.`
    );
  }
   ...
   * and it is what gates `@mica/sdk/core` — the raw NUI transport. Nothing outside
   * micaOS's own repo can be `core: true`: the Store installs `core: false` add-ons and
   * runs them in a sandboxed iframe. micaOS enforces this with a build plugin in a config
  ...
  if (/^\s*core:\s*true\s*,?\s*$/m.test(source)) {
    throw new Error(
      `[mica-addon] ${path.relative(here, MANIFEST)} declares \`core: true\`. An add-on built ` +
        `outside micaOS's own repo is always \`core: false\`: \`core: true\` means the app ships ` +
        `with the phone and cannot be uninstalled, and it is the flag that gates ` +
        `@mica/sdk/core (the raw NUI transport). The Store installs \`core: false\` bundles ` +
  ...
  if (!/^\s*core:\s*false\s*,?\s*$/m.test(source)) {
    throw new Error(
      `[mica-addon] ${path.relative(here, MANIFEST)} does not declare \`core: false\`. It is a ` +
  ...
  if (!id) {
    throw new Error(
      `[mica-addon] could not read \`id: '...'\` from ${path.relative(here, MANIFEST)}. It must be ` +
...
/**
 * `@mica/sdk/core` is refused outright.
 *
  ...
  return {
    ...
    resolveId: {
      ...
      handler(source) {
        ...
        this.error(
          `[mica-addon] an add-on may not import @mica/sdk/core — it is the raw NUI transport, ` +
 ...
 *
 * It matters less than it did, now that `@mica/sdk` resolves to `addon.ts` through the
 ...
 * the shape-matching mistake micaOS's `sdk/seam.test.ts` documents at length; the
 * classification is by what a module imports, not by where it sits.
 */
function requireIframeFacets(): Plugin {
  let seen = false;
  return {
    name: 'mica-addon-require-iframe-facets',
    buildStart() {
      seen = false;
    },
    transform(_code, id) {
    ...
    generateBundle: {
      ...
      handler() {
        if (!seen) {
          this.error(
            `[mica-addon] this bundle does not contain the SDK's iframe facet set ` +
              ...
              `usual cause is \`@mica/sdk\` having resolved to the package's \`index.ts\` ` +
...
/** One capability your code reaches for, and the `@mica/sdk` import that discloses it. */
...
/**
 * The part of micaOS's `sdk/lib/permissionScan.ts` this build uses, plus the table it needs.
 *
...
/**
 * micaOS's own permission derivation, loaded out of the `@mica/sdk` you installed.
 *
...
async function loadPermissionScan(): Promise<SdkPermissionScan> {
  ...
  const entry = await Promise.resolve(import.meta.resolve('@mica/sdk'));
  ...
  const load = async (relative: string): Promise<Record<string, unknown>> => {
    ...
    if (!fs.existsSync(file)) {
      throw new Error(
        `[mica-addon] cannot check this add-on's permissions: ${file} is not there. That ` +
          `file is part of @mica/sdk and this build reads it to learn which imports need ` +
  ...
  if (!table || Object.keys(table).length === 0) {
    throw new Error(
      `[mica-addon] @mica/sdk's permission table came back empty. Comparing against it ` +
  ...
  for (const name of required) {
    if (typeof scan[name] !== 'function') {
      throw new Error(
        `[mica-addon] @mica/sdk's lib/permissionScan.ts does not export ${name}. This ` +
 ...
 *
 * `permissions` is what the Store shows a player before they install your add-on, and micaOS
 ...
 *
 * micaOS's own add-on build runs this same check (MICA-205), and its test suite runs it
 ...
 *
 * The mapping from an `@mica/sdk` import to the permission it discloses is the SDK's, not
 * this file's: a copy of it here would be wrong the first time micaOS adds a hook. So the two
 * source files behind it are loaded out of the installed `@mica/sdk` at build time —
 * `permissions.ts` for the table and `lib/permissionScan.ts` for the derivation, which is the
 * very code micaOS's own build and test suite use.
 *
 ...
 * works, and micaOS's `sdk/lib/permissionScan.test.ts` starts a real Node and imports each of
 * them, so the day that stops being true it fails there rather than here.
 *
 * If micaOS ever moves either file, this build stops with the path it looked for. That is the
...
function requireDeclaredPermissions(): Plugin {
  /** Every name your source imports from `@mica/sdk`, and how many files were scanned. */
  ...
  return {
    ...
    generateBundle: {
      ...
      handler() {
        if (!sdk) {
          this.error(`[mica-addon] the permission scan did not load. This is a bug.`);
        }
        if (scanned === 0) {
          ...
          this.error(
            `[mica-addon] the permission scan saw none of your source files, so it checked ` +
        ...
        if (!declared.ok) {
          this.error(
            `[mica-addon] ${path.relative(here, MANIFEST)}: ${declared.reason} This build ` +
        ...
        if (shortfall.length > 0) {
          this.error(
            `[mica-addon] ${sdk.shortfallMessage(id, path.relative(here, MANIFEST), shortfall)}`
          );
        }
      }
    }
 ...
 * confidently wrong about what it is running on. micaOS shipped exactly that for a while
...
function noUnsubstitutedDefines(): Plugin {
  ...
  return {
    name: 'mica-addon-no-unsubstituted-defines',
    generateBundle: {
      order: 'post',
      handler(_options, bundle) {
        for (const [file, chunk] of Object.entries(bundle)) {
          ...
          if (found.length > 0) {
            this.error(
              `[mica-addon] ${file} still contains unsubstituted build-time identifier(s): ` +
...
export default defineConfig({
  ...
  publicDir: false,
  resolve: {
    // No `@mica/sdk` alias: the package's own `exports` map resolves it to `addon.ts`.
  ...
  build: {
    ...
    emptyOutDir: true,
    /**
     * FiveM's release CEF is Chromium 103 (AGENTS.md §6). `chrome92` is the floor micaOS's
     ...
     * test suite — yours or micaOS's — can catch that.
     */
    target: 'chrome92',
    cssCodeSplit: false,
    /**
