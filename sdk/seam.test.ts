 ...
 *
 * `@mica/sdk` is two things wearing one name (MICA-16). The **kit** — `ui/`, `kit/`,
 ...
 * invisible to it:
 *
 * - `sdk/host/useMail.ts`'s `export { unreadMailCount } from '../../web/src/host/facets/mail'` —
...
const FORBIDDEN_DIRS = [
   ...
   * `typeof import('../../../../web/src/host/facets/mail')`: erased at build time, never resolved.
   *
...
const PATH_ALIASES: [RegExp, string][] = [
  ...
  [/^@mica\/sdk\/app$/, join(SDK, 'app.ts')],
  [/^@mica\/sdk\/core$/, join(SDK, 'core.ts')],
  // Not a package export: it resolves into the consumer. See `web/tsconfig.app.json`.
  [/^@mica\/sdk\/testing$/, join(SRC, 'testing.ts')],
  [/^@mica\/sdk$/, join(SDK, 'index.ts')]
];
...
const isInstalledPackage = (specifier: string): boolean => {
  ...
  // MICA-172: `sdk/` is its own workspace package, so its dependencies (`dompurify`,
  // `marked`, `@material/material-color-utilities`) install into `sdk/node_modules` and
  // no longer appear under `web/`. Checking only the old two locations would make this
  // throw on a perfectly ordinary dependency — which is loud rather than silent, but
  // still wrong. All three are checked; the walk still refuses anything it cannot place.
  return [
    join(SDK, 'node_modules', name),
    join(WEB, 'node_modules', name),
 ...
 * `export { unreadMailCount } from '../../web/src/host/facets/mail'` for a value, which is what
 ...
 */
const ADDON_ALLOWED_SPECIFIERS = [
  '../web/src/services/createCrudStore',
  '../web/src/services/createPagedStore'
];
...
describe('the boot facet sets are exhaustive', () => {
  ...
  it('only an entry point imports the in-process facet set', () => {
     ...
     * backs `@mica/sdk/testing` and stands in for the shell in a unit test.
     *
