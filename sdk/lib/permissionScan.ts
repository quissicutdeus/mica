 ...
 * from `@gphone/sdk`, and fails if a manifest declares less than its imports need. An add-on
...
export interface PermissionShortfall {
  /** The `@gphone/sdk` import that needs it — what the author actually wrote. */
  hook: string;
...
/**
 * A named import from the bare `@gphone/sdk` specifier, in either quote style.
 *
 * Only the bare specifier. `@gphone/sdk/app` publishes `defineApp` and the manifest types,
 * which disclose nothing; `@gphone/sdk/core` is refused to an add-on outright by both add-on
 ...
 */
const SDK_IMPORT = /import\s*(?:[\w$]+\s*,\s*)?\{([^}]*)\}\s*from\s*['"]@gphone\/sdk['"]/g;

/**
 * Every name one file imports from `@gphone/sdk`, sorted.
 *
...
function assertTable(table: PermissionTable): void {
  if (table === null || typeof table !== 'object' || Object.keys(table).length === 0) {
    throw new Error(
      'gPhone permission scan: the permission table is empty or failed to import. Nothing can ' +
 ...
 *
 * A name with no row is not an error: most of what an add-on imports from `@gphone/sdk` is UI
