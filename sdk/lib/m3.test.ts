...
describe('M3 color engine', () => {
  ...
  describe('contrast', () => {
    it('keeps every on-role legible against its own surface', () => {
      ...
      //
      // Every `on-X`/`X` pair the role set declares, not a subset (MICA-109). This
      // listed six of the eleven, which meant `on-secondary`, `on-tertiary`,
      // `on-tertiary-container`, `on-error-container` and `inverse-on-surface` were
      // generated by the same engine as the rest and checked by nothing. All five pass,
      // and the point of writing them down is that they go on passing.
      //
      // Measured worst cases across `SEEDS`, both schemes: 6.42 (`on-primary` on
      // `primary`, light) up to 7.67. The floor below is 4.5.
      for (const seed of SEEDS) {
        const { light, dark } = buildSchemes(seed);
        for (const tokens of [light, dark]) {
    ...
    it('keeps both text roles legible on every surface tier they can land on', () => {
      // The pair above is the *declared* contract — `on-surface` on `surface`. In practice
      // text sits on whichever container tier the component chose, and `surface-container-
      // highest` is four tones away from `surface`. Nothing checked that end of the stack
      // (MICA-109); the worst measured is `on-surface-variant` on
      // `surface-container-highest` at 7.19, so there is real headroom, but it is headroom
      // nobody had counted.
      const TIERS = [
        'surface',
      ...
      for (const seed of SEEDS) {
        const { light, dark } = buildSchemes(seed);
        for (const tokens of [light, dark]) {
          for (const on of ['on-surface', 'on-surface-variant']) {
    ...
    it('keeps the focus ring visible against every surface it can be drawn over', () => {
      // `app.css` paints one ring for the whole phone in `--color-focus-ring`, so it lands
      // on whatever tier the focused control happens to sit on. WCAG wants 3:1 for a
      // non-text indicator; the worst measured is 4.96 (`surface-container-highest`,
      // light), so the margin survives any seed a player picks (MICA-109).
      for (const seed of SEEDS) {
        const { light, dark } = buildSchemes(seed);
        for (const tokens of [light, dark]) {
          for (const base of [
            ...
            const ratio = Contrast.ratioOfTones(lstar(tokens['focus-ring']), lstar(tokens[base]));
            expect(ratio, `${seed} focus-ring on ${base}`).toBeGreaterThanOrEqual(3);
          }
        }
      }
    });
