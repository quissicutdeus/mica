...
describe('clearAppStorage', () => {
  ...
  it('counts the key alongside the value, not the value alone', () => {
     ...
     *
     * A namespaced key is `mica:<appId>:<key>`, which is also the shape of a net event name —
     * so writing one as a literal here trips `eventNames.test.ts`, which scans `web/src` for a
     * borrowed `mica:` prefix. The property under test is that a longer key costs more for the
