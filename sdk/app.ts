...
/**
 * `@mica/sdk/app` — the entry a **manifest** imports, and nothing else.
 *
 * ## Why this exists
 *
 ...
 * registry — so importing `@mica/sdk` loads every app, and every app imports the barrel
 ...
 * ## What goes where
 *
 * - **`@mica/sdk/app`** — a manifest. `defineApp`, `lazyBadge`, and the types either
 *   needs.
 * - **`@mica/sdk`** — everything else an app is built from: hooks, UI, stores. Imported
