 ...
 *   - `lib/moderation.ts` listed `mica_blabber` in the reportable allowlist
 *
...
describe('countFor matches a real reference, not a bare word', () => {
  ...
  it('does not match a quoted string that only contains the id as a substring', () => {
    expect(countFor(`const table = 'mica_notes_dms';`, ['notes'])).toBe(0);
  });
});
...
const IMPORT_BOUNDARY_DIRS = [join(SRC, 'shell'), SDK];
