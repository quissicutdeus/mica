 ...
 *
 * micaOS can run standalone, with no framework resource behind it — and a deployment with
 ...
 * table's, `shared/devices.ts`, republished here so an add-on built against `@mica/sdk`
...
export interface AppManifest {
   ...
   *
   * The stable one. It is a directory name, the `mica:<id>:` storage namespace, the
   ...
   * `'Community'` silently became unremovable, and naming yourself `'micaOS'` was enough to
   ...
   * one of them, which is exactly the failure `@mica/sdk` is built to avoid. Declaring it
...
// `@mica/sdk`'s public surface by `index.ts`'s `export * from './manifest'`. Adding it
...
/**
 * Helper function to define and validate a micaOS application manifest.
...
function resolveTile(id: string, input: { tile?: AppTile; color?: string }): AppTile {
  ...
  if (tile && color !== undefined && color !== flattenTile(tile)) {
    throw new Error(
      `micaOS App Manifest error: '${id}' declares both 'tile' and 'color', and they ` +
  ...
  if (tile) {
    if (!BG_CLASS.test(tile.bg)) {
      throw new Error(
        `micaOS App Manifest error: '${id}' has tile.bg '${tile.bg}', which is not a single ` +
    ...
    if (tile.fg !== undefined && !FG_CLASS.test(tile.fg)) {
      throw new Error(
        `micaOS App Manifest error: '${id}' has tile.fg '${tile.fg}', which is not a single ` +
  ...
  if (typeof color !== 'string' || !color.trim()) {
    throw new Error(
      `micaOS App Manifest error: '${id}' must declare 'tile'. It is what the launcher ` +
  ...
  if (!split) {
    throw new Error(
      `micaOS App Manifest error: '${id}' has color '${color}', which names no 'bg-' ` +
...
export function defineApp(manifest: AppManifestInput): AppManifest {
  if (!manifest.id || typeof manifest.id !== 'string') {
    throw new Error("micaOS App Manifest error: 'id' is required and must be a string.");
  }
  if (manifest.name !== undefined && (typeof manifest.name !== 'string' || !manifest.name)) {
    throw new Error("micaOS App Manifest error: 'name' must be a non-empty string.");
  }
  ...
  /**
   * `ext_` belongs to resources outside micaOS.
   *
   * An external script raises a notification under `ext_<resource>` so it gets its own
   * group in the shade rather than borrowing an app's. That only holds if no micaOS app
  ...
  if (id.startsWith('ext_')) {
    throw new Error(
      `micaOS App Manifest error: id '${id}' uses the reserved 'ext_' prefix, which is ` +
        `for notifications raised by resources outside micaOS. Choose another id.`
    );
  }
  ...
  if (import.meta.env.DEV) {
    if (id !== manifest.id) {
      console.warn(
        `micaOS App Manifest: id '${manifest.id}' is not lowercase and has been read as ` +
          `'${id}'. It is a directory name, a storage namespace and an event segment — ` +
          `spell it lower_snake_case in the manifest.`
      );
    } else if (!ID_PATTERN.test(id)) {
      console.warn(
        `micaOS App Manifest: id '${id}' is not lower_snake_case. ` +
  ...
  if (isRemote && manifest.core === true) {
    throw new Error(
      `micaOS App Manifest error: remote app '${id}' declares 'core: true'. A downloaded ` +
  ...
  if (typeof core !== 'boolean') {
    throw new Error(
      `micaOS App Manifest error: '${id}' must declare 'core'. It decides whether the app ` +
  ...
  if (networkHosts.length > 0 && manifest.requiresNetwork !== true) {
    throw new Error(
      `micaOS App Manifest error: '${id}' declares 'networkHosts' without 'requiresNetwork: ` +
  ...
  for (const host of networkHosts) {
    if (!ORIGIN_PATTERN.test(host)) {
      throw new Error(
        `micaOS App Manifest error: '${id}' declares 'networkHosts' entry '${host}', which ` +
  ...
  if (services !== undefined) {
    if (!Array.isArray(services)) {
      throw new Error(
        `micaOS App Manifest error: '${id}' has a 'services' that is not an array. It names ` +
    ...
    for (const service of services) {
      if (typeof service !== 'string' || !ID_PATTERN.test(service)) {
        throw new Error(
          `micaOS App Manifest error: '${id}' declares service '${String(service)}', which is ` +
      ...
      if (service !== id && !service.startsWith(`${id}_`)) {
        throw new Error(
          `micaOS App Manifest error: '${id}' declares service '${service}', which is outside ` +
  ...
  if (requires !== undefined) {
    if (!Array.isArray(requires)) {
      throw new Error(
        `micaOS App Manifest error: '${id}' has a 'requires' that is not an array. It lists ` +
    ...
    for (const capability of requires) {
      if (!(ALL_CAPABILITIES as readonly string[]).includes(capability)) {
        throw new Error(
          `micaOS App Manifest error: '${id}' declares an unknown capability ` +
  ...
  if (devices !== undefined) {
    if (!Array.isArray(devices)) {
      throw new Error(
        `micaOS App Manifest error: '${id}' has a 'devices' that is not an array. It lists ` +
          `the devices the app appears on — devices: ['phone', 'tablet'].`
      );
    }
    if (devices.length === 0) {
      throw new Error(
        `micaOS App Manifest error: '${id}' declares 'devices: []', which would show it ` +
    ...
    for (const device of devices as readonly unknown[]) {
      if (!(ALL_DEVICES as readonly string[]).includes(device as string)) {
        throw new Error(
          `micaOS App Manifest error: '${id}' declares an unknown device ` +
