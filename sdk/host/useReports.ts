 ...
 * re-exported from `./inProcess/facets/reports`. That re-export was a *value* edge across
 ...
 * plugin decided which. `pendingReportCount` is already a member of the `reports` facet on both
 * sides, so the registry answers it with no new facet name and no permission-table row.
 *
 * `facets.reports()`, not `useReports()`: this export has never been permission-gated
 ...
 */
export const pendingReportCount = lazyBadge(() => facets.reports().pendingReportCount);
